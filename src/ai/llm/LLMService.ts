/**
 * Service LLM — Qwen2.5-1.5B-Instruct (ONNX, local, on-device, embarqué)
 *
 * Modèle embarqué directement dans le bundle natif (assets/ai-models/llm/),
 * quantifié q4f16 (~1.17 Go). Remplace Llama-3.2-1B-Instruct, dont l'asset
 * du release GitHub `youme-ai` était incomplet (poids externes manquants).
 *
 * Architecture Qwen2.5-1.5B (config.json du modèle) :
...
export class LLMService {
  ...
  private async generate(prompt: string, maxNewTokens: number): Promise<string> {
    ...
    const inputIds: number[] = Array.from(encoded.input_ids);
