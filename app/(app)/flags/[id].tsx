...
/**
 * Calcule les scores red/green depuis le résultat Gemini.
 * Avant analyse (result === null) : les deux valent 70 (point de départ neutre).
 */
function computeScores(result: FlagAnalysisResult | null): { greenScore: number; redScore: number } {
  if (!result) return { greenScore: 70, redScore: 70 };

  // greenScore : part de 50 + bonus par green flag selon sévérité
  const greenBonus = result.greenFlags.reduce((acc, f) => {
    return acc + (f.severity === 'élevé' ? 12 : f.severity === 'modéré' ? 8 : 4);
  }, 0);
  const greenScore = Math.min(100, Math.max(0, 50 + greenBonus));

  // redScore : part de 100 - malus par red flag selon sévérité
  // Score élevé = peu/pas de red flags = bonne situation
  const redPenalty = result.redFlags.reduce((acc, f) => {
    return acc + (f.severity === 'élevé' ? 18 : f.severity === 'modéré' ? 10 : 5);
  }, 0);
...
function FlagGauge({
  ...
  const barColor = isGreen
    ? (score >= 70 ? colors.success : score >= 45 ? '#D2A24C' : colors.error)
    : (score >= 70 ? colors.success : score >= 45 ? '#D2A24C' : colors.error);

  const label = isDefault
    ? 'Démarrage — lancer l\'analyse pour affiner'
    : score >= 70 ? 'Bonne situation' : score >= 45 ? 'À surveiller' : 'Préoccupant';

  const styles = useMemo(() => getStyles(colors), [colors]);

  return (
    <View style={[styles.gauge, { marginTop: SPACING.sm }]}>
      <View style={styles.gaugeHeader}>
        <Text style={styles.gaugeTitle}>
          {isGreen ? '✅ Green Flags' : '🚩 Red Flags'}
        </Text>
        <Text style={[styles.gaugeScore, { color: barColor }]}>{score}/100</Text>
      </View>
      <View style={styles.gaugeTrack}>
        <View style={[styles.gaugeFill, { width: `${Math.max(0, Math.min(100, score))}%`, backgroundColor: barColor }]} />
      </View>
      <Text style={[styles.gaugeLabel, { color: barColor }]}>{label}</Text>
    </View>
  );
}
...
export default function FlagsScreen() {
  ...
  const { greenScore, redScore } = useMemo(() => computeScores(result), [result]);
  const isDefault = result === null;
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.scroll}>
      ...
      {/* Jauges toujours visibles — 70% par défaut, mises à jour après analyse */}
      <FlagGauge score={greenScore} type="green" isDefault={isDefault} colors={colors} />
      <FlagGauge score={redScore}   type="red"   isDefault={isDefault} colors={colors} />
