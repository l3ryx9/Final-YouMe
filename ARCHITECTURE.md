...
- Email de vérification obligatoire (lien expirant après 24h)
- Réauthentification pour les actions sensibles (suppression de compte)

### Anti-bot (sans CAPTCHA payant)
- **Honeypot** : champ caché `website` sur les formulaires d'inscription et de
  connexion ; toute valeur non vide bloque la tentative.
- **Délai minimum** : une soumission en moins de 3s après l'ouverture du
  formulaire est considérée comme suspecte.
- **Rate limiting côté serveur** : 5 inscriptions/minute par IP, 10 tentatives
  de connexion/minute par compte (Edge Function `anti-bot-guard` + table
  `rate_limit_events`).
- **Score de risque** combinant honeypot, timing, rate limiting et heuristique
  d'email suspect (domaines jetables) ; décision `allow` / `verify` / `block`.
- **Journalisation** des tentatives bloquées ou suspectes dans
  `security_logs` (IP, date, action, score, raison) pour ajuster les règles.
- Voir `supabase/functions/anti-bot-guard/`, `supabase/migrations/20260715_anti_bot_protection.sql`
  et `src/shared/utils/antiBot.ts`.
